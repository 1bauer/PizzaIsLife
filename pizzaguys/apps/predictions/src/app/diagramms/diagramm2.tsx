...
class diagramm2 extends React.Component<MyProps, MyState> {
  ...
  getTimeSpend(data: Diagrammdata[]){
    
    //group by topic 
    //get topic 
  
    const seriesPerViews:any[]= [];
    data.forEach((e) => {
      const topicExists = seriesPerViews.find(x => x.topic === e.topic );
      if(topicExists === undefined){
        ...
        topicExists.data.push(e.timespend_in_sec);
      }
    })
    return seriesPerViews; 
  }
  ...
  override render() {
   ...
   if(this.state.data != undefined){
    ...
    series: this.getTimeSpend(this.state.data),}
